...
const server = http.createServer((req, res) => {
  ...
  let fp;
  if (safe === 'index.html' || safe === '/index.html') {
    // 兼容两种上传方式：优先仓库根目录的 index.html，找不到再用 public/index.html
    const rootIdx = path.join(__dirname, 'index.html');
    const pubIdx = path.join(PUBLIC_DIR, 'index.html');
    fp = (fs.existsSync(rootIdx) ? rootIdx : pubIdx);
  } else {
    fp = path.join(PUBLIC_DIR, safe);
